...
export async function runLiveNode(document: FlowDocument, nodeId: string, api: CatbotsDesktopApi['nodes'], market: string) {
  ...
  const snapshot = await loadMarket(api, market, [...new Set(nodes.filter(node => ['data.candles', 'data.candle_items'].includes(node.type)).map(node => String(node.config.timeframe)))]);
