...
export function StrategyGraph({ revision, onSelectNode }: StrategyGraphProps) {
  ...
  return (
    <section className="strategy-graph" aria-label={`Strategy flow for ${revision.name}`}>
      <ReactFlow
        nodes={graph.nodes}
        edges={graph.edges}
        nodeTypes={nodeTypes}
...
function StrategyNodeCard({ data }: NodeProps<StrategyFlowNode>) {
  return (
    <div className={`strategy-node strategy-node-${data.kind}`} aria-label={data.accessibleName}>
