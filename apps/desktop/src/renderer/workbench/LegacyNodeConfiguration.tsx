...
export function LegacyNodeConfiguration({ node, revision, disabled, onSave, nodeApi }: {
  nodeApi?: CatbotsDesktopApi['nodes']; node: Node; revision: StrategyRevision; disabled?: boolean; onSave?: (config: Record<string, unknown>) => Promise<void>;
}) {
  ...
  return <div className="legacy-node-config">
    <Button size="sm" loading={running} disabled={running || dirty || disabled || !node.config || !market.trim()} onClick={async () => {
      setTab('debug'); setResult(null); setError(''); setSource(''); setRunning(true);
      try {
    ...
    <Tabs tabs={[{ value: 'config', label: 'Configuration' }, { value: 'debug', label: 'Data & debug' }]} value={tab} onValueChange={setTab} />
    {error && <Banner variant="error" title="Could not update node" description={error} />}
    {tab === 'config' ? <>
      ...
      {source && <p>{source}</p>}
      {result && <>
        <Badge variant={result.selected.status === 'executed' ? 'success' : 'info'}>{result.selected.status}</Badge>
        <p>{result.trace.length} nodes evaluated · {result.market}</p>
        <h3>Input</h3><pre>{JSON.stringify(result.selected.inputs, null, 2)}</pre>
        <h3>Output</h3><pre>{JSON.stringify(result.selected.outputs, null, 2)}</pre>
        <h3>Execution path</h3>{result.trace.map(item => <p key={item.nodeId}>{item.nodeId} · {item.status}</p>)}
      </>}
