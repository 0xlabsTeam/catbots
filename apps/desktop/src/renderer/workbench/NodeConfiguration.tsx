...
export function NodeConfiguration({ draft, node, disabled, onSave, onClose, onDebug, initialRun, nodeApi, workspace: sharedWorkspace, onSelectNode, connections }: {
  ...
  return <NodeDetailLayout header={<header className="node-detail-header">
    ...
    <section className="node-run-context"><h4>Run context</h4><Input size="sm" label="Run market" value={market} disabled={running} onChange={event => workspace.setMarket(event.target.value)} /><p>Hyperliquid mainnet · real prices and closed candles. Account equity is unavailable.</p>
