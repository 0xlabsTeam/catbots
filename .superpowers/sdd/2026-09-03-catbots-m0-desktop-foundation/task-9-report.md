...
## Fix round 3 — isolated restoration process

The host-ABI restoration `npm` child now receives the same `onSpawn` tracking as rebuild and Forge. On POSIX it runs in a detached process group with ignored terminal stdio; on Windows it uses ignored stdio and `windowsHide` without POSIX-only grouping. The parent always retains and awaits the child exit, and the signal controller observes restoration but never forwards `SIGINT`/`SIGTERM` to it. A signal during restoration waits for the one memoized restore promise, then exits conventionally; a restore failure reports the existing fixed nonzero diagnostic. Tests assert POSIX/Windows spawn options, a simulated terminal SIGTERM cannot kill the restoration child, restoration completes once, listeners are removed, already-exited child waits resolve, and failure behavior remains covered (130 desktop tests).

The remaining unavoidable OS boundary is an abrupt parent termination that prevents JavaScript cleanup from running at all (for example `SIGKILL` or host crash): the detached restore child may complete independently, but the parent cannot await it. Normal `SIGINT`/`SIGTERM`, package failure, and restoration failure are handled and awaited by the parent. Node 22 full tests/typecheck, package-backed E2E, make, deep verifier, and diff check passed after this change.

## Fix round 4 — Windows restoration isolation

Windows host-ABI restoration now combines `detached: true`, `windowsHide: true`, and ignored stdio, so the `npm.cmd` restore child is created independently of the parent's console. The POSIX detached process-group behavior is unchanged. The focused regression was RED because the captured Windows spawn options omitted `detached`, then GREEN after the one-line option fix. A separate lifecycle test proves the parent signal controller does not forward `SIGTERM` to the restoration child, waits for that child to exit, completes the memoized restore once, removes both listeners, and only then exits 143.

Node 22 validation: the focused orchestration suite passed 14 tests; the full suite passed 3 contracts tests and 131 desktop tests; typecheck passed; `pnpm make` produced the darwin/arm64 ZIP and completed the deep package verifier; a separate verifier invocation also exited 0. The package phase of `pnpm test:e2e` succeeded and its tray lifecycle test passed. The First Launch test reached its heading assertion successfully, but its existing Playwright `app.close()` cleanup call then hung until the 30-second test timeout; an isolated rerun reproduced the same cleanup timeout. The trace places the timeout in `e2e/cleanup.ts:19`, after launch and visibility passed, so no fully green E2E claim is made for this round.

## Fix round 5 — bounded Playwright shutdown

Playwright close is now bounded by a five-second promise race before cleanup inspects the Electron child again. A close timeout or rejection checks process exit immediately, sends `SIGKILL` only while the child is still alive, and waits at most another five seconds for forced exit. An already-exited process skips Playwright close entirely, preserving the confirmed native-Quit lifecycle. The documented cleanup contract treats a verified forced exit as successful recovery because Catbots intentionally remains alive in the tray; unrecovered close failures, a child that remains alive after forced termination, and temporary-directory removal failures are still reported explicitly. Multiple failures are preserved in an `AggregateError`, and the isolated data directory is removed once from `finally` even when shutdown fails or a late close promise subsequently settles.

TDD evidence under Node 22: the never-resolving close test first timed out at Vitest's five-second ceiling against the unbounded implementation. The forced-exit-timeout test then failed on the old generic forced-termination error, and the compound-failure test initially lost the post-kill timeout. After implementation, the focused `e2e-cleanup.test.ts` suite passed 9 tests without wall-clock sleeps. Coverage includes never-settling and late-settling close promises, close rejection, already-exited and normally exiting processes, successful forced recovery, bounded forced-exit failure, one-time directory removal, and compound error preservation.

Final Node 22 validation: `pnpm typecheck` passed; `pnpm test` passed 3 contracts tests and 138 desktop tests. Two consecutive `pnpm test:e2e` runs each passed both Playwright tests; the First Launch cases completed in 6.5 seconds and the tray lifecycle cases in 991ms and 989ms, respectively. The first sandboxed packaging attempt failed explicitly on `getaddrinfo ENOTFOUND github.com`; the network-enabled rerun and the consecutive stability run both passed. `pnpm make` produced `apps/desktop/out/make/zip/darwin/arm64/@catbots-desktop-darwin-arm64-0.1.0.zip` and completed its deep verifier. A separate `pnpm exec node scripts/verify-package-contents.mjs apps/desktop/out` invocation also exited 0. `git diff --check` passed before this report update.
